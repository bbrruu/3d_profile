...
async function renderPortfolioTab() {
  ...
  const prices  = {};
  symbols.forEach(function(s) { prices[s] = null; });
  ...
  holdings.forEach(function(h) {
    ...
    if (pnl.value > 0) {
      segments.push({ label: h.symbol, value: pnl.value });
      totalNTD += pnl.value;
    }
  });
...
function renderSummaryCards(holdings, prices, fxRate) {
  ...
  function setCardClass(id, n) {
    const el = document.getElementById(id);
    if (el) { el.className = 'card-value ' + colorClass(n); }
  }

  setCard('port-networth', fmtTWD(netWorth));
  setCard('port-invested', fmtTWD(totalCost));
  setCard('port-pnl',  fmtTWD(totalPnl));
  setCardClass('port-pnl', totalPnl);
  setCard('port-pnlpct', fmtPct(totalPct));
  setCardClass('port-pnlpct', totalPct);
  setCard('port-positions', holdings.length);
}
...
function renderHoldingsTable(holdings, prices, fxRate) {
  ...
  tbody.innerHTML = holdings.map(function(h) {
    ...
    return '<tr>' +
      ...
      '<td><button class="btn btn-sm btn-danger" onclick="deletePosition(\'' + h.id + '\')">Remove</button></td>' +
      '</tr>';
  }).join('');
}
...
function renderTransactions() {
  ...
  tbody.innerHTML = txns.slice(0, 50).map(function(tx) {
...
function deletePosition(id) {
  const portfolio = getPortfolio();
  portfolio.holdings = (portfolio.holdings || []).filter(function(h) { return h.id !== id; });
  savePortfolio(portfolio);
...
async function refreshPortfolioPrices() {
  const portfolio = getPortfolio();
  const symbols   = (portfolio.holdings || []).map(function(h) { return h.symbol; });
  if (symbols.length === 0) return;

  showToast('Refreshing prices...', 'info');
  try {
    const quotes = await fetchMultipleQuotes(symbols);
    if (window.APP) window.APP.prices = Object.assign(window.APP.prices || {}, quotes);
    renderPortfolioTab();
...
/* ── P&L Calculator ─────────────────────────────────────── */
function calcHoldingPNL(holding, currentPrice, fxRate) {
  const fx = fxRate ? fxRate.twdPerUsd : 31.5;
  const toNTD = function(price) {
    return holding.currency === 'USD' ? price * fx : price;
  };
  ...
  if (currentPrice == null) {
    return { value: cost, cost, pnl: 0, pnlPct: 0 };
  }

  const priceNTD = toNTD(currentPrice);
  const value    = priceNTD * holding.shares;
